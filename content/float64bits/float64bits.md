...
绝对精度是指一个float64数值与其最接近的可表示的float64数值之间的差值的绝对值，这个值和ULP是等价的。正规数的绝对精度随着数值的增加而变得越来越低，随着数值的减小而变得越来越高；次正规数的绝对精度是一个非常小的、远超正规数精度的常数。

相对精度是该数值绝对值和ULP和之比。该值越大，表示精度越高；该值越小，表示精度越低。

正规数的相对精度为

$RP_{Normal} = \frac{Value}{ULP_{Normal}} = \frac{(1.f) × 2^E}{2^{E-52}}= \frac{1.f}{2^{-52}}$

当尾数`f`为0时，相对精度达到最小值$2^{52}$；当`f`接近1时，相对精度接近最大值$2^{53}$。整体来看，正规数的相对精度基本恒定。

与此对应，次正规数的相对精度为

$RP_{Subnormal} = \frac{Value}{ULP_{Subnormal}} = \frac{(0.f)×2^{-1074}}{2^{-1074}}={0.f}$

可见，次正规数的相对精度等于其自身，且在值接近0时精度也趋近于0。因此次正规数的相对精度不是一个特别有意义的比较指标。
