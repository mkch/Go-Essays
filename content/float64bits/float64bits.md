...
ULP是指两个相邻的float64数值之间的差值的绝对值。ULP较小，表示精度较高；ULP较大，表示精度较低。
...
从这个公式可以看出，指数相等的正规数ULP也相等。指数每增加1，ULP会翻倍，精度则减半；指数每减少1，ULP会减半，精度则翻倍。

用类似的方法可以推导出次正规数的ULP为：

$ULP_{Subnormal}(x) = 2^{-1074}$

从这个公式可以看出次正规数的ULP是一个非常小的常数，因此次正规数的精度非常高，可以用来表示非常接近0的数值。

### ULP距离

ULP距离是指两个float64数值之间的ULP数量。

对于两个非无穷大、非NaN的float64数值$x$和$y$，如果它们的位模式分别为$u_x$和$u_y$，则它们之间的ULP距离可以通过以下代码计算：
...
- 如果float64数值的符号位为1（负数），则对其位模式取反，使得所有负数依次映射到0～0x7FFFFFFFFFFFFFFF区间内，其中`-0`被映射到0x0x7FFFFFFFFFFFFFFF；
...
这里面还有一个边界条件需要注意：`-0`被映射到0x0x7FFFFFFFFFFFFFFF，而`+0`被映射到0x8000000000000000，它俩之间的ULP距离为1而不是0。
